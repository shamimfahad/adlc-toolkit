...
1. Determine the toolkit installation path. Resolve `~/.claude/skills/init/` back to its junction/symlink target. The `init` folder is a junction to `<toolkit-root>/init`, so the toolkit root is the parent of that target. Call this `$TOOLKIT_PATH`.
...
Ask in chat (or via AskUserQuestion if available):
